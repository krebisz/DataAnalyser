...
public static class CmsTypeConverter
{
    ...
    /// </summary>
    public static ICanonicalMetricSeries ToConsumerCms<TValue>(CanonicalMetricSeries<TValue> internalCms)
    {
        if (internalCms == null)
        ...
        var timeSemantics = new TimeSemantics(internalCms.TimeAxis.IsIntervalBased ? TimeRepresentation.Interval : TimeRepresentation.Point, internalCms.Timestamps.Count > 0 ? internalCms.Timestamps[0] : DateTimeOffset.MinValue, internalCms.Timestamps.Count > 0 ? internalCms.Timestamps[internalCms.Timestamps.Count - 1] : null);

        // Convert timestamps and values to MetricSamples
        var samples = new List<MetricSample>();
        for (var i = 0; i < internalCms.Timestamps.Count && i < internalCms.Values.Count; i++)
        {
        ...
        if (internalCms.Dimensions.Dimensions.TryGetValue("Dimension", out var dimStr))
            dimension = Enum.TryParse<MetricDimension>(dimStr, true, out var parsed) ? parsed : MetricDimension.Unknown;
        ...
        // Build provenance
        var provenance = new MetricProvenance(internalCms.Provenance.TryGetValue("SourceProvider", out var provider) ? provider : "Unknown", internalCms.Provenance.TryGetValue("SourceDataset", out var dataset) ? dataset : "Unknown", internalCms.Provenance.TryGetValue("NormalizationVersion", out var version) ? version : "Unknown");
    ...
    /// </summary>
    public static IReadOnlyList<ICanonicalMetricSeries> ToConsumerCmsList<TValue>(IReadOnlyList<CanonicalMetricSeries<TValue>> internalCmsList)
    {
