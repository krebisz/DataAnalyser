...
public static class SamsungHealthParser
{
    ...
    private static List<HealthMetric> ParseMeasurementObject(JToken measurement, string filePath, string metricType)
    {
        ...
        if (measurement is not JObject obj)
            return metrics;
        ...
        foreach (var property in obj.Properties())
        {
            ...
            if (propName.Contains("time", StringComparison.OrdinalIgnoreCase) || propName.Contains("date", StringComparison.OrdinalIgnoreCase))
                continue;

            // Extract numeric values
            if (propValue.Type == JTokenType.Float || propValue.Type == JTokenType.Integer)
                if (decimal.TryParse(propValue.ToString(), out var value))
                {
                    ...
                    foreach (var otherProp in obj.Properties())
                        if (otherProp.Name != propName && !otherProp.Name.Contains("time", StringComparison.OrdinalIgnoreCase))
