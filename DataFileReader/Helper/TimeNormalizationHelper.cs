...
public static class TimeNormalizationHelper
{
    ...
    public static DateTime? ParseTimestamp(object? timestampValue)
    {
        ...
        if (long.TryParse(timestampValue.ToString(), out var longValue))
        {
            ...
            if (longValue > 946684800) // Jan 1, 2000 in seconds
                return ConvertUnixSeconds(longValue);
        }

        // Try as DateTime string (ISO 8601, etc.)
        if (DateTime.TryParse(timestampValue.ToString(), out var dateTime))
    ...
    /// </summary>
    public static DateTime? ExtractTimestamp(JToken token, string preferredField = "start_time")
    {
        if (token == null)
            return null;
