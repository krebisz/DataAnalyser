...
{
    public CanonicalMetricSeries(MetricIdentity metric, TimeAxis timeAxis, DimensionSet dimensions, IReadOnlyList<DateTimeOffset> timestamps, IReadOnlyList<TValue?> values, IReadOnlyDictionary<string, string> provenance)
    {
    ...
    private void ValidateStructure()
    {
        ...
        if (Timestamps.Count == 0)
            throw new ArgumentException("CanonicalMetricSeries must contain at least one data point.");
    }
}