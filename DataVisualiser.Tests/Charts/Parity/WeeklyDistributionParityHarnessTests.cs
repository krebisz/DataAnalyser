...
public class WeeklyDistributionParityHarnessTests
{
    ...
    public void WeeklyDistribution_Legacy_vs_Cms_Parity_Passes_On_ExtendedResult()
    {
        ...
        var cms = TestDataBuilders.CanonicalMetricSeries().WithMetricId("metric.body_weight").WithStartTime(start).WithInterval(TimeSpan.FromDays(1)).WithValue(100m).WithUnit("kg").WithSampleCount(10).Build();
