...
namespace DataVisualiser.Tests.Charts.Parity
{
    public class WeeklyDistributionParityHarnessTests
    {
        ...
        public void WeeklyDistribution_Legacy_vs_Cms_Parity_Passes_On_ExtendedResult()
        {
            ...
            var to = start.AddDays(9).DateTime;
            ...
            // Act
            var parity = harness.Validate(
                new StrategyParityContext
                {
                    StrategyName = "WeeklyDistribution",
                    MetricIdentity = "weight",
                    Mode = ParityMode.Strict,
                    Tolerance = new ParityTolerance { AllowFloatingPointDrift = true, ValueEpsilon = 1e-9 }
                },
                legacyExecution: () => WeeklyDistributionParityHarness.ToLegacyExecutionResult(legacyStrategy.ExtendedResult),
                cmsExecution: () => WeeklyDistributionParityHarness.ToCmsExecutionResult(cmsStrategy.ExtendedResult));
