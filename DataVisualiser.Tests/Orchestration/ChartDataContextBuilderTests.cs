...
public sealed class ChartDataContextBuilderTests
{
    ...
    public void Build_ShouldAlignAndComputeDerivedValues()
    {
        ...
        Assert.Equal(new[] { From.Date, From.AddDays(1).Date, From.AddDays(2).Date }, ctx.Timestamps);
        Assert.Equal(new[] { 10.0, 10.0, 30.0 }, ctx.RawValues1);
        Assert.Equal(new[] { 0.0, 5.0, 5.0 }, ctx.RawValues2);
        Assert.Equal(new[] { 10.0, 5.0, 25.0 }, ctx.DifferenceValues);
        Assert.Equal(new[] { 0.0, 2.0, 6.0 }, ctx.RatioValues);
        Assert.Equal(new[] { 10.0 / 30.0, 10.0 / 30.0, 1.0 }, ctx.NormalizedValues1);
        Assert.Equal(new[] { 0.0, 1.0, 1.0 }, ctx.NormalizedValues2);
    ...
    public void Build_ShouldSetDisplayNamesAndSemanticCount()
    {
        var data = TestDataBuilders.HealthMetricData().
                                    WithTimestamp(From).
                                    BuildSeries(1, TimeSpan.FromDays(1));
