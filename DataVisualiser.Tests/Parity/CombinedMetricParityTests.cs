    ...
    public class CombinedMetricParityTests
    {
            ...
            var result = _harness.Validate(
                ...
                legacyExecution: () =>
                {
                    var legacyStrategy = new CombinedMetricStrategy(
                        ...
                        from,
                        to);
                    return legacyStrategy.Compute()?.ToLegacyExecutionResult()
                        ?? new LegacyExecutionResult { Series = Array.Empty<ParitySeries>() };
                },
                cmsExecution: () =>
                {
                    var cmsStrategy = new CombinedMetricStrategy(
                        ...
                        from,
                        to);
                    return cmsStrategy.Compute()?.ToCmsExecutionResult()
                        ?? new CmsExecutionResult { Series = Array.Empty<ParitySeries>() };
                });

            // Assert
            Assert.True(result.Passed,
                $"Parity failed: {string.Join("; ", result.Failures.Select(f => $"[{f.Layer}] {f.Message}"))}");
        }
        ...
        public void Parity_ShouldPass_WithMismatchedCounts()
        {
            ...
            var context = new StrategyParityContext
            {
                ...
                MetricIdentity = "Left|Right",
                Mode = ParityMode.Diagnostic,
                Tolerance = new ParityTolerance { AllowFloatingPointDrift = true }
            };
