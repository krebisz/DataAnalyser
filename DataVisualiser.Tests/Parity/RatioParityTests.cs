...
public sealed class RatioParityTests
{
    [Fact]
    public void Parity_ShouldPass_ForCmsAndLegacy()
    {
        ...
        var legacyRight = TestDataBuilders.HealthMetricData().WithTimestamp(from).WithValue(20m).WithUnit("kg").BuildSeries(10, interval);
        ...
        var cmsRight = TestDataBuilders.CanonicalMetricSeries().WithMetricId("metric.right").WithStartTime(new DateTimeOffset(from)).WithInterval(interval).WithValue(20m).WithUnit("kg").WithSampleCount(10).Build();

        var legacyResult = new RatioStrategy(legacyLeft, legacyRight, "Left", "Right", from, to).Compute();
        var cmsResult = new RatioStrategy(cmsLeft, cmsRight, "Left", "Right", from, to).Compute();
    ...
    private static void AssertEquivalent(ChartComputationResult? legacyResult, ChartComputationResult? cmsResult)
    {
        ...
        {
            var expected = legacyResult.PrimaryRawValues[i];
            var actual = cmsResult.PrimaryRawValues[i];
            if (double.IsNaN(expected) && double.IsNaN(actual))
                continue;

            Assert.True(Math.Abs(expected - actual) < 0.0001, $"Raw mismatch at {i}");
