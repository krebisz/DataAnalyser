...
namespace DataVisualiser.Tests.Parity
{
    ...
    public class SingleMetricParityTests
    {
        ...
        public void Parity_ShouldPass_WithIdenticalData()
        {
            ...
            // Compare timestamps
            for (int i = 0; i < legacyResult.Timestamps.Count; i++)
            {
                Assert.Equal(legacyResult.Timestamps[i], cmsResult.Timestamps[i]);
            }

            // Compare values (with tolerance for floating-point)
            for (int i = 0; i < legacyResult.PrimaryRawValues.Count; i++)
            {
                ...
                Assert.True(Math.Abs(legacyVal - cmsVal) < 0.0001,
                    $"Value mismatch at index {i}: legacy={legacyVal}, cms={cmsVal}");
            }
        }
        ...
        public void Parity_ShouldPass_WithNullValues()
        {
            ...
            var legacyData = new List<DataVisualiser.Models.HealthMetricData>
            {
                ...
                new() { NormalizedTimestamp = from.Add(interval * 2), Value = 102m, Unit = "kg" }
            };
