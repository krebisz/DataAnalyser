...
namespace DataVisualiser.Tests.Services
{
    public sealed class MetricSelectionServiceTests
    {
        ...
        // ---- Test helper ----
        private static (SamplingMode Mode, int? TargetSamples, int? MaxRecords)
            InvokeResolveStrategy(
                MetricSelectionService service,
                DateTime from,
                DateTime to,
                long recordCount)
        {
            // ResolveDataLoadStrategy is private — invoke via reflection
            var method = typeof(MetricSelectionService)
                .GetMethod("ResolveDataLoadStrategy", System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.NonPublic);

            Assert.NotNull(method);

            return ((SamplingMode Mode, int? TargetSamples, int? MaxRecords))
                method!.Invoke(service, new object[] { from, to, recordCount })!;
        }
