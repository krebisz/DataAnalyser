...
namespace DataVisualiser.Tests.Services
{
    ...
    public sealed class StrategySelectionServiceTests
    {
        ...
        public StrategySelectionServiceTests()
        {
            ...
            _service = new StrategySelectionService(
                _mockCutOverService.Object,
                _mockParityService.Object,
                ConnectionString);
        }
        ...
        public void SelectComputationStrategy_ShouldReturnSingleMetricStrategy_WhenOneSeries()
        {
            ...
            var labels = new List<string> { "Series1" };
            var ctx = new ChartDataContext { SemanticMetricCount = 1 };
        ...
        public void SelectComputationStrategy_ShouldReturnCombinedMetricStrategy_WhenTwoSeries()
        {
            ...
            var labels = new List<string> { "Series1", "Series2" };
            var ctx = new ChartDataContext { SemanticMetricCount = 2 };
            var from = DateTime.UtcNow.AddDays(-10);
            var to = DateTime.UtcNow;
        ...
        public void SelectComputationStrategy_ShouldReturnMultiMetricStrategy_WhenThreeOrMoreSeries()
        {
            ...
            var labels = new List<string> { "Series1", "Series2", "Series3" };
            var ctx = new ChartDataContext { SemanticMetricCount = 3 };
            var from = DateTime.UtcNow.AddDays(-10);
            var to = DateTime.UtcNow;
        ...
        public void SelectComputationStrategy_ShouldThrowNullReferenceException_WhenSeriesIsNull()
        {
            ...
            Assert.Throws<NullReferenceException>(() =>
                _service.SelectComputationStrategy(null!, labels, ctx, from, to));
        }
        ...
        public void LoadAdditionalSubtypesAsync_ShouldReturnEarly_WhenSubtypesCountIsTwoOrLess()
        {
            ...
            var selectedSubtypes = new List<string?> { "subtype1", "subtype2" };

            // Act
            var task = _service.LoadAdditionalSubtypesAsync(
                series, labels, "Weight", "HealthMetrics",
                DateTime.UtcNow.AddDays(-10), DateTime.UtcNow, selectedSubtypes);
            task.Wait();
        ...
        public void LoadAdditionalSubtypesAsync_ShouldReturnEarly_WhenMetricTypeIsNullOrEmpty()
        {
            ...
            var series = new List<IEnumerable<HealthMetricData>>();
            var labels = new List<string>();
            var selectedSubtypes = new List<string?> { "subtype1", "subtype2", "subtype3" };

            // Act
            var task1 = _service.LoadAdditionalSubtypesAsync(
                series, labels, null, "HealthMetrics",
                DateTime.UtcNow.AddDays(-10), DateTime.UtcNow, selectedSubtypes);
            task1.Wait();

            var task2 = _service.LoadAdditionalSubtypesAsync(
                series, labels, string.Empty, "HealthMetrics",
                DateTime.UtcNow.AddDays(-10), DateTime.UtcNow, selectedSubtypes);
            task2.Wait();
