...
namespace DataVisualiser.Tests.Strategies
{
    public sealed class NormalizedStrategyTests
    {
        ...
        public void Compute_ShouldReturnResult_WithNaNs_WhenNoOverlappingTimestamps()
        {
            ...
            {
                new() { NormalizedTimestamp = To.AddDays(5), Value = 20m, Unit = "kg" }
            };
            ...
            var left = new List<HealthMetricData>
    {
        ...
        new() { NormalizedTimestamp = From.AddDays(3), Value = 40m, Unit = "kg" },
        new() { NormalizedTimestamp = From.AddDays(4), Value = 50m, Unit = "kg" }
    };

            var right = new List<HealthMetricData>
    {
        new() { NormalizedTimestamp = From, Value = 5m, Unit = "kg" }
    };
        ...
        public void Compute_ShouldReturnTwoSeries_ForRelativeToMax()
        {
            ...
            {
                new() { NormalizedTimestamp = From, Value = 5m, Unit = "kg" },
                new() { NormalizedTimestamp = From.AddDays(1), Value = 10m, Unit = "kg" }
            };
        ...
        public void Compute_ShouldGenerateSmoothedSeries()
        {
            ...
            var right = new List<HealthMetricData>
            {
                new() { NormalizedTimestamp = From, Value = 5m, Unit = "kg" },
                new() { NormalizedTimestamp = From.AddDays(1), Value = 10m, Unit = "kg" },
                new() { NormalizedTimestamp = From.AddDays(2), Value = 15m, Unit = "kg" },
                new() { NormalizedTimestamp = From.AddDays(3), Value = 20m, Unit = "kg" }
            };
        ...
        public void SecondaryLabel_ShouldBeSet_ForRelativeToMax()
        {
            var strategy = new NormalizedStrategy(
                ...
                Enumerable.Empty<HealthMetricData>(),
                "L", "R", From, To, NormalizationMode.RelativeToMax);
