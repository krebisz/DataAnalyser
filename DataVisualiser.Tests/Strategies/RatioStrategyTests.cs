...
namespace DataVisualiser.Tests.Strategies
{
    public sealed class RatioStrategyTests
    {
        ...
        public void Compute_ShouldCalculateLeftDividedByRight()
        {
            ...
            {
                new() { NormalizedTimestamp = From, Value = 2m, Unit = "kg" },
                new() { NormalizedTimestamp = From.AddDays(1), Value = 4m, Unit = "kg" }
            };
            ...
            Assert.NotNull(result);
            Assert.Equal(new[] { 5.0, 5.0 }, result!.PrimaryRawValues);
        }
        ...
        public void Compute_ShouldProduceNaN_WhenRightValueIsZero()
        {
            ...
            var right = new List<HealthMetricData>
            {
                new() { NormalizedTimestamp = From, Value = 0m, Unit = "kg" }
            };
