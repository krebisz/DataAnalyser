...
public sealed class TransformResultStrategyTests
{
    ...
    public void Compute_ShouldPreserveTimestamps()
    {
        ...
        var strategy = new TransformResultStrategy(data, values, "X", From, To);

        var result = strategy.Compute();

        Assert.NotNull(result);
        Assert.Equal(data.Select(d => d.NormalizedTimestamp), result!.Timestamps);
    }
    ...
    public void Compute_ShouldSetPrimaryLabel()
    {
        ...
        var strategy = new TransformResultStrategy(data, values, "Transformed(A)", From, To);
