...
public sealed class EvidenceDistributionParityEvaluatorTests
{
    [Fact]
    public async Task BuildAsync_WithoutChartContext_ReturnsUnavailable()
    {
        var evaluator = CreateEvaluator();

        var result = await evaluator.BuildAsync(new ChartState(), new MetricState(), null);

        Assert.Equal("Unavailable", result.Status);
        Assert.Equal("No chart context available", result.Reason);
    }

    [Fact]
    public async Task BuildAsync_WithoutResolvableSelection_ReturnsUnavailable()
    {
        var evaluator = CreateEvaluator();
        ...
        var result = await evaluator.BuildAsync(new ChartState(), new MetricState(), context);

        Assert.Equal("Unavailable", result.Status);
        Assert.Equal("No distribution series selected", result.Reason);
    }

    private static EvidenceDistributionParityEvaluator CreateEvaluator()
    {
        ...
        return new EvidenceDistributionParityEvaluator(service, () => null);
    }
