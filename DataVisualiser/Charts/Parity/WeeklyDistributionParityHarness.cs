...
namespace DataVisualiser.Charts.Parity
{
    ...
    public sealed class WeeklyDistributionParityHarness : IStrategyParityHarness
    {
            ...
            for (int i = 0; i < legacy.Series.Count; i++)
            {
                ...
                for (int p = 0; p < l.Points.Count; p++)
                {
                    ...
                    if (lp.Time != cp.Time)
                    {
                        return Fail(
                            ParityLayer.TemporalParity,
                            $"Time mismatch at index {p} in series '{l.SeriesKey}': legacy={lp.Time:o}, cms={cp.Time:o}",
                            context);
                    }
        ...
        public static LegacyExecutionResult ToLegacyExecutionResult(WeeklyDistributionResult? ext) =>
            new() { Series = BuildParitySeries(ext) };
        ...
        private static IReadOnlyList<ParitySeries> BuildParitySeries(WeeklyDistributionResult? ext)
        {
            ...
            DateTime T(int idx) => baseDate.AddDays(idx);
            ...
            series.Add(new ParitySeries
            {
                ...
                {
                    new() { Time = baseDate, Value = ext.GlobalMin },
                    new() { Time = baseDate.AddDays(1), Value = ext.GlobalMax }
                }
            });
        ...
        private static void AddArraySeries(
            List<ParitySeries> series,
            string key,
            IReadOnlyList<double>? values,
            Func<int, DateTime> timeAt)
        {
            if (values == null)
                return;
            ...
            series.Add(new ParitySeries
            {
                SeriesKey = key,
                Points = values
                    .Select((v, i) => new ParityPoint { Time = timeAt(i), Value = v })
                    .ToList()
            });
        }
