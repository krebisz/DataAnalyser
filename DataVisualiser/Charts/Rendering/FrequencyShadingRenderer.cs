    ...
    int intervalIndex)
            ...
            double[] cumulativeStackHeight)
        {
            ...
            for (int dayIndex = 0; dayIndex < 7; dayIndex++)
            {
                ...
                bool overlaps = dayRange > 0 && interval.Min < dayMax && interval.Max > dayMin;
                ...
                int frequency = frequenciesPerDay.TryGetValue(dayIndex, out var df) &&
                                df.TryGetValue(intervalIndex, out var f)
                                ? f : 0;
            ...
            List<double> ranges)
        {
            ...
            {
                double dayMin = SafeMin(mins, day);
                double dayMax = dayMin + SafeRange(ranges, day);
        ...
        private static double SafeRange(List<double> ranges, int i)
            => double.IsNaN(ranges[i]) || ranges[i] < 0 ? 0.0 : ranges[i];
