...
public sealed class CombinedMetricCmsStrategy : IChartComputationStrategy
{
    ...
    public CombinedMetricCmsStrategy(ICanonicalMetricSeries left, ICanonicalMetricSeries right, string labelLeft, string labelRight, DateTime from, DateTime to, ITimelineService? timelineService = null, ISmoothingService? smoothingService = null, IUnitResolutionService? unitResolutionService = null)
    {
    ...
    public ChartComputationResult? Compute()
    {
        ...
        var (timestamps, primaryRaw, secondaryRaw) = AlignSeries(leftOrdered, rightOrdered);
        ...
        var (primarySmoothed, secondarySmoothed) = SmoothSeries(leftOrdered, rightOrdered, timestamps);
    ...
    private (List<DateTime> Timestamps, List<double> Primary, List<double> Secondary) AlignSeries(List<CmsPoint> left, List<CmsPoint> right)
    {
        var count = Math.Min(left.Count, right.Count);
        if (count == 0)
            return (new List<DateTime>(), new List<double>(), new List<double>());

        var leftTuples = left.Select(p => (p.Timestamp, p.ValueDecimal)).
            ToList();
        var rightTuples = right.Select(p => (p.Timestamp, p.ValueDecimal)).
            ToList();
    ...
    private (List<double> Primary, List<double> Secondary) SmoothSeries(List<CmsPoint> left, List<CmsPoint> right, List<DateTime> timestamps)
    {
    ...
    private static List<CmsPoint> FilterAndOrder(ICanonicalMetricSeries cms, DateTime from, DateTime to)
    {
        return cms.Samples.Where(s => s.Value.HasValue).
            Select(s => new CmsPoint(s.Timestamp.UtcDateTime, // DateTimeOffset -> DateTime
                s.Value // decimal?
            )).
