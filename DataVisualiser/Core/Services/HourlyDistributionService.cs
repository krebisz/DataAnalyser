...
public class HourlyDistributionService
{
    ...
    private const    bool                                       UseCmsBucketDistribution = false;
    ...
    public async Task UpdateDistributionChartAsync(CartesianChart targetChart, IEnumerable<MetricData> data, string displayName, DateTime from, DateTime to, double minHeight = DefaultMinHeight, bool useFrequencyShading = true, int intervalCount = 10, ICanonicalMetricSeries? cmsSeries = null, bool enableParity = false)
    {
        ...
        var (result, frequencyResult) = await ComputeBucketDistributionAsync(data, cmsSeries, displayName, from, to, useCmsStrategy, enableParity);
        ...
        try
        {
            ...
            SetupBucketTooltip(targetChart, result, frequencyResult, useFrequencyShading, intervalCount);
    ...
    ///     This shows baseline (min) + range (max-min) as stacked columns per bucket.
    /// </summary>
    private void RenderOriginalMinMaxChart(CartesianChart targetChart, ChartComputationResult result, string displayName, double minHeight, BucketDistributionResult? frequencyData, bool useFrequencyShading = true, int intervalCount = 25)
    {
        ...
        LogBucketSummary(mins, ranges, bucketValues, globalMin, globalMax);
    ...
    private void LogBucketSummary(List<double> mins, List<double> ranges, Dictionary<int, List<double>> bucketValues, double globalMin, double globalMax)
    {
    ...
    /// </summary>
    private Dictionary<int, List<double>> GetBucketValuesFromStrategy(BucketDistributionResult? frequencyData)
    {
    ...
    private async Task<(ChartComputationResult? Result, BucketDistributionResult? ExtendedResult)> ComputeBucketDistributionAsync(IEnumerable<MetricData> data, string displayName, DateTime from, DateTime to)
    {
        // Backwards-compatible default (legacy-only)
        return await ComputeBucketDistributionAsync(data, null, displayName, from, to, false, false).
                ConfigureAwait(true);
    }

    private async Task<(ChartComputationResult? Result, BucketDistributionResult? ExtendedResult)> ComputeBucketDistributionAsync(IEnumerable<MetricData> data, ICanonicalMetricSeries? cmsSeries, string displayName, DateTime from, DateTime to, bool useCmsStrategy, bool enableParity)
    {
    ...
    ///// <summary>
    ///// Calculates simple tooltip data for Simple Range mode (min, max, range, count per bucket).
    ///// </summary>
    ///// <summary>
    /// <summary>
    ///     Sets up tooltip for weekly distribution chart.
    /// </summary>
    private void SetupBucketTooltip(CartesianChart targetChart, ChartComputationResult result, BucketDistributionResult extendedResult, bool useFrequencyShading, int intervalCount)
    {
