namespace DataVisualiser.Helper
{
    ...
    public static class FrequencyBinningHelper
    {
        ...
        public static double CalculateBinSize(double minValue, double maxValue)
        {
            ...
            double targetBinCount = 15.0;
            double rawBinSize = range / targetBinCount;
        ...
        public static List<(double Min, double Max)> CreateBins(double minValue, double maxValue, double binSize)
        {
            ...
            {
                double binMin = current;
                double binMax = current + binSize;
        ...
        /// </summary>
        public static Dictionary<int, int> BinValuesAndCountFrequencies(
            List<double> values,
            List<(double Min, double Max)> bins)
        {
            ...
            foreach (var value in values)
            {
                ...
                int binIndex = FindBinIndex(value, bins);
        ...
        /// </summary>
        public static Dictionary<int, Dictionary<int, double>> NormalizeFrequencies(
            Dictionary<int, Dictionary<int, int>> frequenciesPerDay)
        {
            ...
            foreach (var kvp in frequenciesPerDay)
            {
                ...
                {
                    int binIndex = binFreq.Key;
                    int frequency = binFreq.Value;
                    double normalizedFreq = (double)frequency / globalMaxFreq;
