...
namespace DataVisualiser.Helper
{
    ...
    public static class StrategyComputationHelper
    {
                ...
                DateTime to)
        {
            ...
            var ordered1 = left?.Where(d => d.Value.HasValue).OrderBy(d => d.NormalizedTimestamp).ToList() ?? new List<HealthMetricData>();
            var ordered2 = right?.Where(d => d.Value.HasValue).OrderBy(d => d.NormalizedTimestamp).ToList() ?? new List<HealthMetricData>();
        ...
        /// </summary>
        public static List<DateTime> CombineTimestamps(
            IEnumerable<HealthMetricData> ordered1,
            IEnumerable<HealthMetricData> ordered2)
        {
        ...
        /// </summary>
        public static (List<double> RawValues1, List<double> RawValues2) ExtractAlignedRawValues(
            List<DateTime> combinedTimestamps,
            Dictionary<DateTime, double> dict1,
            Dictionary<DateTime, double> dict2)
        {
            var rawValues1 = combinedTimestamps
                .Select(ts => dict1.TryGetValue(ts, out var v1) ? v1 : double.NaN)
                .ToList();

            var rawValues2 = combinedTimestamps
                .Select(ts => dict2.TryGetValue(ts, out var v2) ? v2 : double.NaN)
                .ToList();
        ...
        /// </summary>
        public static (List<double> InterpSmoothed1, List<double> InterpSmoothed2) ProcessSmoothedData(
            List<HealthMetricData> ordered1,
            List<HealthMetricData> ordered2,
            List<DateTime> combinedTimestamps,
            DateTime from,
