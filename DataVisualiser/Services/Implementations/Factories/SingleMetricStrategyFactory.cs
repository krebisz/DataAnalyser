...
public sealed class SingleMetricStrategyFactory : IStrategyFactory
{
    ...
    {
        return new SingleMetricCmsStrategy(ctx.PrimaryCms as ICanonicalMetricSeries ?? throw new InvalidOperationException("PrimaryCms is null"), parameters.Label1, parameters.From, parameters.To);
    }

    public IChartComputationStrategy CreateLegacyStrategy(StrategyCreationParameters parameters)
    {
        return new SingleMetricLegacyStrategy(parameters.LegacyData1 ?? Array.Empty<HealthMetricData>(), parameters.Label1, parameters.From, parameters.To);
    }
}