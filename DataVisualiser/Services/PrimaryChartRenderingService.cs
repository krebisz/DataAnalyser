...
public sealed class PrimaryChartRenderingService
{
    ...
    public PrimaryChartRenderingService(ChartUpdateCoordinator chartUpdateCoordinator, WeeklyDistributionService weeklyDistributionService, IStrategyCutOverService strategyCutOverService, string connectionString)
    {
        _orchestrator = new ChartRenderingOrchestrator(chartUpdateCoordinator ?? throw new ArgumentNullException(nameof(chartUpdateCoordinator)), weeklyDistributionService ?? throw new ArgumentNullException(nameof(weeklyDistributionService)), strategyCutOverService ?? throw new ArgumentNullException(nameof(strategyCutOverService)));
    ...
    /// </summary>
    private (List<IEnumerable<HealthMetricData>> series, List<string> labels) BuildInitialSeriesList(IEnumerable<HealthMetricData> data1, IEnumerable<HealthMetricData>? data2, string displayName1, string displayName2)
    {
