...
public sealed class TransformOperationService
{
    ...
    /// </summary>
    public TransformOperationResult ComputeUnaryTransform(
        ...
        var metricsList = new List<IReadOnlyList<HealthMetricData>>
    {
        ...
        {
            Debug.WriteLine(
                $"[Transform] UNARY - Using NEW infrastructure for operation: {operation}");

            computedResults =
                TransformExpressionEvaluator.Evaluate(expression, metricsList);

            Debug.WriteLine(
                $"[Transform] UNARY - Evaluated {computedResults.Count} results");
        }
        else
        {
            Debug.WriteLine(
                $"[Transform] UNARY - Using LEGACY approach for operation: {operation}");
            ...
            var values =
                preparedData.Select(d => (double)d.Value!.Value).ToList();

            computedResults =
                MathHelper.ApplyUnaryOperation(values, op);
        }
    ...
    /// </summary>
    public TransformOperationResult ComputeBinaryTransform(
     ...
     string operation)
    {
        ...
        var (aligned1, aligned2) =
            TransformExpressionEvaluator.AlignMetricsByTimestamp(
                prepared1,
                prepared2);
        ...
        var metricsList = new List<IReadOnlyList<HealthMetricData>>
    {
        ...
        {
            Debug.WriteLine(
                $"[Transform] BINARY - Using NEW infrastructure for operation: {operation}");

            computedResults =
                TransformExpressionEvaluator.Evaluate(expression, metricsList);

            Debug.WriteLine(
                $"[Transform] BINARY - Evaluated {computedResults.Count} results");
        }
        else
        {
            Debug.WriteLine(
                $"[Transform] BINARY - Using LEGACY approach for operation: {operation}");
            ...
            var values1 =
                aligned1.Select(d => (double)d.Value!.Value).ToList();
            var values2 =
                aligned2.Select(d => (double)d.Value!.Value).ToList();

            computedResults =
                MathHelper.ApplyBinaryOperation(values1, values2, op);
        }
