...
namespace DataVisualiser.UI.Charts.Presentation.ECharts;
...
public sealed class EChartsChartRenderer : IChartRenderer
{
    public Task ApplyAsync(IChartSurface surface, UiChartRenderModel model, CancellationToken cancellationToken = default)
    {
        ...
        if (surface is ITrackedChartContentSurface trackedContentSurface)
            trackedContentSurface.SetHasRenderedContent(model.HasRenderableContent);
