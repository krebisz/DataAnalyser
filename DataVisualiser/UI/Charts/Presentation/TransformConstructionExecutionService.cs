...
internal sealed class TransformConstructionExecutionService(MetricLoadSnapshotGateway gateway)
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (steps.Count == 0)
            throw new ArgumentException("At least one transform construction step is required.", nameof(steps));
        if (string.IsNullOrWhiteSpace(title))
            throw new ArgumentException("Transform construction title cannot be empty.", nameof(title));
