...
public sealed class EvidenceExportWriter
{
    ...
    public Result Write(object payload, string targetDirectory, DateTime utcNow, string fileNamePrefix = "evidence")
    {
        if (payload == null)
        ...
        var filePath = Path.Combine(targetDirectory, $"{fileNamePrefix}-{utcNow:yyyyMMdd-HHmmss}.json");
